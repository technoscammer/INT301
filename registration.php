...
$username = $emailid = $password = $confirm_password = "";
$username_err = $emailid_err = $password_err = $confirm_password_err = "";
...
// Validate email
    if(empty(trim($_POST["emailid"]))){
        $username_err = "Please enter a valid emailid.";
    } else{
        // Prepare a select statement
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $emailid_err = "This email is already taken.";
                } else{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sss", $param_username, $param_password, $param_emailid);
